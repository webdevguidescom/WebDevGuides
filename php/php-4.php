...
$result = mysqli_query($con,"SELECT count FROM likes WHERE ID = 5");
$data = $result->fetch_all(MYSQLI_ASSOC);

$result2 = mysqli_query($con,"SELECT views FROM likes WHERE ID = 5");
$data2 = $result2->fetch_all(MYSQLI_ASSOC);


if(!isset($_COOKIE['not_unique5'])) {
    setcookie('not_unique5', '1', time() + (86400)); //30 days
    ...
    $sql = "UPDATE likes SET views = views + 1 WHERE ID = 5";
...
<html lang="en-US" dir="ltr" data-navigation-type="combo" data-navbar-horizontal-shape="slim">
  ...
  <head>
    ...
    <meta name="description" content="Learn how to set up email functionality on your Nginx web server hosted on Amazon Lightsail, and also verify the email
    with md5 HASH using PHP">

    <!-- ===============================================-->
    <!--    Document Title-->
    <!-- ===============================================-->
    <title>Configuring Email on an Nginx Web Server with Amazon Lightsail and PHP. Learn how to set up email functionality on your Nginx web server hosted on Amazon Lightsail.  to verify the email
      with md5 HASH | Computer Science X</title>
                        ...
                        <li class="nav-item"><a class="nav-link dropdown-indicator" href="#php-1" data-bs-toggle="collapse" aria-expanded="true" aria-controls="php-1">
                          ...
                          <div class="parent-wrapper">
                            <ul class="nav collapse parent show" data-bs-parent="#php" id="php-1">
                              ...
                              <li class="nav-item"><a class="nav-link active" href="/php/php-4.php">
              ...
              <div class="card">
                ...
                <div class="card-body p-2 p-sm-2 p-lg-3">
                  ...
                  <header>
                      ...
                      <nav aria-label="...">
                        ...
                        <ul class="pagination justify-content-center">
                          ...
                          <li class="page-item active" aria-current="page">
                            <a class="page-link" href="/php/php-4.php">4 <span class="sr-only"></span></a>
                          </li>
                    ...
                    <h1 class="display-5">Setting Up Email Functionality on Nginx Web Server: A Guide with PHP Verification</h1>
                    <p class="lead">Learn how to set up email functionality on your Nginx web server hosted on Amazon Lightsail, and also verify the email
                      with md5 HASH using PHP</p>
...
<h5 id="GFG"></h5>
                    ...
                    <div class="container">
                      ...
                      <ol>
                        <li class="h5"><a href="#a1" style="color:teal;font-size: 16px;">Download the SMTP files required</a></li>
                        <li class="h5"><a href="#a2" style="color:teal;font-size: 16px;">PHP</a></li>
                        <li class="h5"><a href="#a3" style="color:teal;font-size: 16px;">MySQL Workbench intrucrions</a></li>
                        <li class="h5"><a href="#a4" style="color:teal;font-size: 16px;">Add users email to MySQL DB for subscriptions and then give that entry to the database a verify value</a></li>
                      </ol>
                    </div>
                    ...
                    <ol>
                      <li id="a1"> 
                        <p>First, you will need to download the SMTP PEAR module files required. Refer to <a href="https://docs.bitnami.com/aws/infrastructure/nginx/troubleshooting/send-mail/" target="_blank" rel="noopener noreferrer" style="color:teal;"><strong>this page</strong></a> for instructions on setting it up on your NginX AWS Lightsail web server.</p>
                      </li>
                      <hr class="hr" />
                      ...
                      <ol>
                        ...
                        <ul>
                          ...
                          <li>Open <a href="/php/php-3.php" target="_blank" rel="noopener noreferrer" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Guide for setting up AWS MySQL Database using MySQL Workbench" style="color:teal;">MySQL Workbench</a> and connect to your MySQL server.</li>
...
CREATE DATABASE csx2;
                        ...
                        <ul>
                          ...
                          <li>Enter the table name (e.g., "email").</li>
                          <li>Add columns: "id", "email" and "verified" with their respective data types and any other necessary details.</li>
...
USE csx2; SELECT count FROM likes WHERE pageName = 'page1';
                    ...
                    <hr class="hr" />
                      ...
                      <li id="a3"><p>Extending Email Subscription Code from <a href="/php/php-2.php" target="_blank" rel="noopener noreferrer" style="color:teal;">section 2</a>, we can enhance our email subscription functionality. Here’s how it works:</p>

                        <ol>
                          <li><strong>User Submission:</strong> When a user submits their email, we store it in our database for email subscriptions.</li>
                          <li><strong>Verification Process:</strong> Each entry in the database is assigned a verification value (a boolean, either 0 or 1). Initially, this value is set to 0.</li>
                          <li><strong>User Verification:</strong> When the user clicks the verification link in their email inbox, they are directed to the verify.php page.</li>
                          <li><strong>Comparison:</strong> Our PHP code on the verify.php page compares the MD5 hash of the user’s email with the stored hash.</li>
                          <li><strong>Update:</strong> Update: If the hashes match, we change the verification value for that email to 1.</li>
                        </ol>
                        <br>
                        <p>This approach ensures secure and verified email subscriptions.</p>
                        <mark><strong>action_page.php</strong></mark>    
...
?&gt;
                      ...
                      <li id="a4">
                        <p>Now create a php file called <mark>verify.php</mark> and add the following code(ensuring that you change the variable values to your own):</p>

                        <mark><strong>verify.php</strong></mark>    
<pre class="rounded-3 language-bash tabindex=0">
<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-filetype-php" viewBox="0 0 16 16">
<path fill-rule="evenodd" d="M14 4.5V14a2 2 0 0 1-2 2h-1v-1h1a1 1 0 0 0 1-1V4.5h-2A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v9H2V2a2 2 0 0 1 2-2h5.5zM1.6 11.85H0v3.999h.791v-1.342h.803q.43 0 .732-.173.305-.175.463-.474a1.4 1.4 0 0 0 .161-.677q0-.375-.158-.677a1.2 1.2 0 0 0-.46-.477q-.3-.18-.732-.179m.545 1.333a.8.8 0 0 1-.085.38.57.57 0 0 1-.238.241.8.8 0 0 1-.375.082H.788V12.48h.66q.327 0 .512.181.185.182.185.522m4.48 2.666V11.85h-.79v1.626H4.153V11.85h-.79v3.999h.79v-1.714h1.682v1.714zm.703-3.999h1.6q.433 0 .732.179.3.175.46.477.158.302.158.677t-.161.677q-.159.299-.463.474a1.45 1.45 0 0 1-.733.173H8.12v1.342h-.791zm2.06 1.714a.8.8 0 0 0 .084-.381q0-.34-.184-.521-.184-.182-.513-.182h-.66v1.406h.66a.8.8 0 0 0 .375-.082.57.57 0 0 0 .237-.24Z"/>
</svg>
...
if (isset($_GET['code'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        header("Location: https://computersciencex.com/indexVerify.php", true, 301);  
...
<div class="float-start">          <a href="/php/php-3.php" style="color:teal; font-size: 20px; " class="rounded float-left"><strong>Previous page:</strong> How to create an AWS MySQL Database and connect to it via MySQL Workbench</a>
</div>
<div class="float-end">          <a href="/php/php-5.php" style="color:teal; font-size: 20px; " class="rounded float-right"><strong>Next page:</strong> How to create a page like button that increments by +1 every time a user clicks and displays the MySQL data on HTML page, thats retreived from the database using PHP</a>
                ...
                <div class="d-none d-lg-block"><!--hide on screens smaller than lg-->
                  ...
                  <nav aria-label="...">
                    <ul class="pagination justify-content-center">
                      <li class="page-item">
                        <a class="page-link" href="/php/php-3.php" tabindex="-1" aria-disabled="true">Previous</a>
                      </li>
                      <li class="page-item"><a class="page-link" href="/php/php-1.php">1</a></li>
                      ...
                      <li class="page-item"><a class="page-link" href="/php/php-5.php">5</a></li>
                      <li class="page-item">
                        <a class="page-link" href="/php/php-5.php">Next</a>
                      </li>
